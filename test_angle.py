...
#
#     return length
def find_length(length_test, length_standard, length_now=math.sqrt(170 ** 2 + 257 ** 2),
                length_calibration=math.sqrt(170 ** 2 + 257 ** 2)):
...
def find_and_draw_outer_black_rectangle(thresh, image, draw_output=False, show_output=False, epsilon_rate=0.02,
                ...
                if len(leftovers) == 0:
                    ...
                    if len(concave_points) == 4:
                        ...
                        if draw_output:
                            ...
                            cv2.putText(output, f"pingxing:{distance}", longest_pair[0][0], cv2.FONT_HERSHEY_SIMPLEX,
                                        0.5, (255, 0, 255), 1)
                    ...
                    if len(concave_points) == 4:
                        ...
                        if xianglian_list.count(False) == 2:
                            ...
                            if draw_output:
                                ...
                                cv2.putText(output, f"distance:{distance}", p2, cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                            (255, 0, 255), 1)
                    ...
                    if is_side_contrast(binary_img, leftovers[0], leftovers[1]):
                        ...
                        points, center = find_full_square_by_one_side(leftovers[0], leftovers[1], distance,
                                                                      concave_points)
                    ...
                    # 对角线
                        ...
                        if draw_output:
                            cv2.putText(output, f"distance:{distance_genhao2}", leftovers[0], cv2.FONT_HERSHEY_SIMPLEX,
                                        0.5,
                                        (255, 0, 255), 1)
...
def filter_and_average(return_list, distance_threshold=20, length_threshold=10):
    ...
    for item in return_list:
        ...
        for group in groups:
            ...
            if np.linalg.norm(np.array(center) - np.array(avg_center)) < distance_threshold and abs(
                    length - avg_length) < length_threshold:
                group.append(item)
                matched = True
                break
...
while True:
    ...
    try:
        if len(diagonals) > 0 and len(width_height_ratio) > 0:
            ...
            print("rdrrdttyyt",width_height_ratio_mean)
            Angle = angle_xiaozhun(width_height_ratio_mean, find_distance(diagonal_mean))
            print("角度", Angle)
            # cmd = f"Angle={int(Angle * 100)}"  # 构造命令字符串
            # cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
            # ser_screen.write(cmd_bytes)  # 发送数据到串口
            # time.sleep(0.03)
            # ser_screen.write(cmd_bytes)  # 发送数据到串口
