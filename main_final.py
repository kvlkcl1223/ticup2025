...
#ser_screen = open_serial('COM20', 115200, timeout=0.1)
ser_screen=[]
# cap = open_camera()
...
# question=1基础题 2：发挥前两题 3：数字题 4：角度题 5：调试模式
question = 4
number = "1"
...
def maintask():
    while True:
        ...
        if question == 1:
            ...
            cmd = f"Disatance={int(find_distance(diagonal_mean) * 1000.0)}"  # 构造命令字符串
            cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
            ser_screen.write(cmd_bytes)  # 发送数据到串口
            time.sleep(0.1)

            # 使用 struct 打包为 4 字节小端 float
            data_bytes = struct.pack('<f', round(X / 10.0, 3))  # IEEE 754 格式，小端字节序
