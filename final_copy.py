...
# 设置 GPIO14 为 ALT4 + 上拉
subprocess.run(["pinctrl", "set", "14", "a4", "pu"], check=True)

# 验证设置
result = subprocess.run(["pinctrl", "get", "14"], capture_output=True, text=True)
print("GPIO14 当前状态:", result.stdout)
...
def find_and_draw_outer_black_rectangle(thresh,image,draw_output=False,show_output=False,epsilon_rate=0.02,length_threshold_rectangular_frame=30):
    output = image.copy()
    ...
    if best_rect is not None:
        ...
        # 设置边缘厚度为2像素，颜色为纯白
        thickness = 2
        white_color = 255 if len(cropped_img.shape) == 2 else (255, 255, 255)

        # 顶部边
        cropped_img[0:thickness, :] = white_color
        # 底部边
        cropped_img[h - thickness:h, :] = white_color
        # 左侧边
        cropped_img[:, 0:thickness] = white_color
        # 右侧边
        cropped_img[:, w - thickness:w] = white_color
...
###判断是否是正方形的边
def is_side_contrast(binary_img, pt1, pt2, step=5, side_length=5, threshold_ratio=0.5):
    """
...
def find_and_draw_outer_black_rectangle_rotate(thresh, image, draw_output=False, show_output=False,
                                               length_threshold_rectangular_frame=30):
    output = image.copy()
...
def find_and_draw_outer_black_rectangle_rotate_q5(thresh, image, draw_output=False, show_output=False,
                                               length_threshold_rectangular_frame=30):
    output = image.copy()
                ...
                if len(leftovers) == 0:
                    ...
                    if len(concave_points)==4:
                        ...
                        if xianglian_list.count(False) == 2:
                            ...
                            print("不应该出现的情况或未考虑的情况")
                    elif len(concave_points)==3:
                        filtered_points = [
                            point_temp for point_temp in concave_points
                            if not is_side_contrast(binary_img, leftovers[0], point_temp)
                        ]
                        print(filtered_points)
                        p1=[]
                        p2=[]
                        if len(filtered_points) == 2:
                            p1= filtered_points[0]
                            p2 = filtered_points[1]
                        else:
                            p1 = concave_points[0]
                            p2 = concave_points[1]
                        print(p1)
                        ...
                        if p1 is not None and p2 is not None:
                            distance = distance_point_to_line(leftovers[0], p1, p2)

                            if draw_output:
                                # 画线（假设 thickness=2）
                                cv2.line(output, leftovers[0], p1, (127, 0, 255), 2)
                                cv2.line(output, leftovers[0], p2, (127, 0, 255), 2)  # 画对角线

                                cv2.putText(output, f"distance:{distance}", leftovers[0], cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                            (255, 0, 255), 1)

                            vertices, center = find_square_vertices(leftovers[0], p1, p2, distance)
...
def xiaozheng_rec(diagonal, length,distance):
    k = 1.0135+0.01389*(distance-1.00)
    X = find_length(length, diagonal)*k
...
class FastestDet:
    def __init__(self, confThreshold=0.5, nmsThreshold=0.4, drawOutput=False):
        ...
        path_names = r"/home/Hust_HWD/Desktop/ti_cup/my.names"  # 识别类别
        path_onnx = r"/home/Hust_HWD/Desktop/ti_cup/my.onnx"
...
def open_serial(port, baudrate, timeout=None, retry_interval=10):
    """
    尝试打开串口，直到成功为止。
...
def parse_received_data(received_data):
    """
    提取形如 'xx=x!' 的字符串中的包头和数字
    """
    # 确保字符串符合格式
    if '=' in received_data and '!' in received_data:
        try:
            header, rest = received_data.split('=')
            value = rest.split('!')[0]
...
def parse_received_data_float(received_data):
    """
    提取形如 'xx=1.23!' 的字符串中的包头和数字（支持 float 类型）
    """
    if '=' in received_data and '!' in received_data:
        try:
            header, rest = received_data.split('=', 1)
            value_str = rest.split('!', 1)[0]
            return header.strip(), float(value_str)
        except ValueError:
            print("格式错误，无法解析数字:", received_data)
            return None, None
    else:
        print("格式不匹配:", received_data)
        return None, None
...
def process_command(cmd):
    ...
    if cmd.startswith("add") and "=" in cmd:
        ...
        if idx_str.isdigit() and val_str.lstrip("-").isdigit():
            idx = int(idx_str)
            delta = int(val_str)
            modify_param_by_index(r"/home/Hust_HWD/Desktop/ti_cup/params.txt", idx, delta)
        else:
...
def angle_xiaozhun(ratio, distance):
    x = ratio
    # 多项式系数
    B0 = -157.17535
    B1 = 166.96572
...
ser_st =  open_serial('/dev/ttyAMA2', 115200, timeout=0.1)
ser_screen = open_serial('/dev/ttyAMA0', 115200, timeout=0.1)
ser_tiaoshi = open_serial('/dev/ttyAMA3', 115200, timeout=0.1)
ser_st.flushInput()
ser_screen.flushInput()
ser_tiaoshi.flushInput()
#ser_screen=[]
cap = None
index = -1
...
while cap is None:
    try:
        cap, index = open_camera(0, 10)
    except Exception as e:
        print("没有找到摄像头，3秒后重试...")
        time.sleep(3)

print(f" 摄像头 {index} 已就绪，开始读取画面")
...
list_lunci=0
width_height_ratio=[]
# question=1基础题 2：发挥前两题 3：数字题 4：角度题 5：其他 6：调试
question = 0
number = "-1"

cmd = f"start=1"  # 构造命令字符串
cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
ser_screen.write(cmd_bytes)  # 发送数据到串口
...
def main_task():
    global ser_screen,ser_st,ser_tiaoshi,cap,i,radiuses,triangle_lengths,rectangle_lengths,diagonals,return_list_all,number_list,list_lunci,question,number,current,max_current,power,max_power,width_height_ratio
    while True:
        while True:
            try:
                if ser_screen.in_waiting > 0:  # 检查是否有数据等待读取
                    # 读取一行数据并解码
                    print("来自屏幕有信息发送")
                    try:

                        received_data = ser_screen.readline().decode('ascii').strip()
                        print("received_data", received_data)
                        header,data = parse_received_data(received_data)
                        if header == "Q":
                            question=data
                            cmd = f"Get=1"  # 构造命令字符串
                            cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                ...
                if ser_tiaoshi.in_waiting > 0:  # 检查是否有数据等待读取
                    # 读取一行数据并解码
                    print("来自屏幕有信息发送")
                    try:

                        received_data = ser_tiaoshi.readline().decode('ascii').strip()
                        print("received_data", received_data)
                        header,data = parse_received_data(received_data)
                        if header == "Q":
                            question=data
                ...
                ser_screen = open_serial('', 115200, timeout=0.1)
                print("已重新打开")
            time.sleep(0.05)

        params = read_params(r"/home/Hust_HWD/Desktop/ti_cup/params.txt")
        print(params)
        black_threshold_min = params.get("black_min", 0)
        black_threshold_max = params.get("black_max", 90)
        epsilon = params.get("epsilon", 20)/1000.0
        print(epsilon)
        if question == 1:
            ...
            while time.time() - start_time < 3:
                ...
                if ret:
                    ...
                    frame = frame[300:1080, 620:1240]
                    # frame 原图像 gray灰度图 thresh 二值图
                    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

                    thresh = cv2.inRange(gray, black_threshold_min, black_threshold_max)
                    # 开闭运算去噪
                    # kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
                    # thresh = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel)
                    # thresh = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, kernel)

                    res = find_and_draw_outer_black_rectangle(thresh, frame, False, False,epsilon)

                    if res is not None:
                        # print("检测到矩形外框")

                        gray_res = cv2.cvtColor(res[0], cv2.COLOR_BGR2GRAY)
                        thresh_res = cv2.inRange(gray_res, black_threshold_min, black_threshold_max)
                        # 获取图像尺寸
                        ...
                        radius = find_circles(gray_res, thresh_res, res[0], False, False)
                        if radius is not None:
                            radiuses.append(radius)
                        triangle_length = find_triangle_points(thresh_res, res[0], False, False)
                        if triangle_length is not None:
                            triangle_lengths.append(triangle_length)
                        rectangle_length = find_rectangle_points(thresh_res, res[0], False, False)
            ...
            try:
                if len(diagonals)>0:
                    ...
                    print("D=", round(find_distance(diagonal_mean) * 100.0, 3))
                    cmd = f"Distance={int(find_distance(diagonal_mean) * 1000.0)}"  # 构造命令字符串
                    cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                    ser_screen.write(cmd_bytes)  # 发送数据到串口
                    time.sleep(0.02)
                    ser_screen.write(cmd_bytes)  # 发送数据到串口
                    ...
                    print("X:", round(X / 10.0, 3))
                    cmd = f"Length={int(X * 100.0)}"  # 构造命令字符串
                    cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                    ser_screen.write(cmd_bytes)  # 发送数据到串口
                    time.sleep(0.02)
                    ser_screen.write(cmd_bytes)  # 发送数据到串口
            ...
            while time.time() - start_time < 3:
                ...
                if ret:
                    ...
                    if res is not None:
                        ...
                        return_list1 = find_contour_points_new(thresh_res, res[0], False, False)
                        ...
                        if count %8 == 0 and return_list1 is not None:
                            break
            filtered = filter_and_average(return_list_all)
            if filtered is not None and len(filtered) > 0:

                min_item = min(filtered, key=lambda x: x[2])
                min_length = min(filtered, key=lambda x: x[2])[2]
                print(f"最小值: , 最小长度: {min_length}")
                diagonal_mean = np.mean(diagonals)
                X = xiaozheng_rec(diagonal_mean, min_length, find_distance(diagonal_mean))
                ...
                if question==3:
                    match = next((item for item in number_list if item[0] == number), None)
                    if match is not None:
                        print("找到匹配项:", match)
                        X=xiaozheng_rec(diagonal_mean,match[1], find_distance(diagonal_mean))
                    else:
                        print("未找到匹配项")
                        min_item = min(number_list, key=lambda x: x[1])
                        X = xiaozheng_rec(diagonal_mean, min_item[1], find_distance(diagonal_mean))

                print("x=,", X)
                print("D=", round(find_distance(diagonal_mean) * 100.0, 3))
                # data_bytes = struct.pack('<f', round(find_distance(diagonal_mean) * 100.0, 3))  # IEEE 754 格式，小端字节序
                cmd = f"Distance={int(find_distance(diagonal_mean) * 1000.0,)}"  # 构造命令字符串
                cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                print("distance",cmd_bytes)
                ser_screen.write(cmd_bytes)  # 发送数据到串口
                time.sleep(0.1)
                ser_screen.write(cmd_bytes)  # 发送数据到串口
                time.sleep(0.02)
                ...
                print("X:", round(X / 10.0, 3))
                cmd = f"Length={int(X*100)}"  # 构造命令字符串
                cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                print(cmd_bytes)
                ser_screen.write(cmd_bytes)  # 发送数据到串口
                time.sleep(0.02)
            ...
            while time.time() - start_time < 3:
                ...
                if ret:
                    ...
                    res = find_and_draw_outer_black_rectangle_rotate(thresh, frame, False, False)
                    ...
                    if res is not None:
                        ...
                        # if triangle_length is not None:
                        #     triangle_lengths.append(triangle_length)
                        rectangle_length = find_rectangle_points_rotate(thresh_res, res[0], False, False)
                        if rectangle_length is not None:
                            rectangle_lengths.append(rectangle_length)
            ...
            while time.time() - start_time < 3:
                ...
                if ret:
                    ...
                    res = find_and_draw_outer_black_rectangle_rotate_q5(thresh, frame, False, False)
                    ...
                    if res is not None:
                        ...
                        if res[1] != 0 and res[2] != 0:
                            width_height_ratio.append(res[1] / res[2])
                        ...
                        if count % 10 == 0:
                            break
            try:
                if len(diagonals) > 0 and len(width_height_ratio)>0:
                    diagonal_mean = np.mean(diagonals)
                    print("距离,", find_distance(diagonal_mean))
                    width_height_ratio_mean = np.mean(width_height_ratio)

                    Angle = angle_xiaozhun(width_height_ratio_mean,find_distance(diagonal_mean))
                    print("角度", Angle)

                    cmd = f"Angle={int(Angle * 1000)}"  # 构造命令字符串
                    cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                    ser_screen.write(cmd_bytes)  # 发送数据到串口
                    time.sleep(0.03)
                    ser_screen.write(cmd_bytes)  # 发送数据到串口
            ...
            send_params_to_screen(r"/home/Hust_HWD/Desktop/ti_cup/params.txt", ser_tiaoshi)
            params = read_params(r"/home/Hust_HWD/Desktop/ti_cup/params.txt")
            print(params)
            black_threshold_min_debug = params.get("black_min", 0)
            black_threshold_max_debug = params.get("black_max", 90)
            epsilon_debug = params.get("epsilon", 20)/1000.0
            while True:
                try:
                    if ser_tiaoshi.in_waiting > 0:
                        data = ser_tiaoshi.readline().decode('utf-8').strip()
                        if data == "Q=0!":
                            question = 0
                ...
                if ret:
                    ...
                    # 设置窗口位置 (x, y)
                    cv2.moveWindow("frame", 200, 200)  # 将窗口移动到屏幕坐标(100,200)
                    # 显示图片
                    cv2.imshow("frame", cv2.resize(frame, (320, 320)))

                    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                    thresh = cv2.inRange(gray,black_threshold_min_debug,black_threshold_max_debug)

                    # 创建命名窗口
                    cv2.namedWindow("thresh", cv2.WINDOW_NORMAL)
                    # 设置窗口位置 (x, y)
                    cv2.moveWindow("thresh", 200, 600)  # 将窗口移动到屏幕坐标(100,200)
                    # 显示图片
                    cv2.imshow("thresh", cv2.resize(thresh, (320, 320)))


                    res = find_and_draw_outer_black_rectangle(thresh, frame, True, True, epsilon_debug)
                time.sleep(0.5)
        try:
            ser_screen.flushInput()
            start_time = time.time()
            while time.time()-start_time<0.3:
                if ser_screen.in_waiting > 0:
                    try:
                        receive_data = ser_screen.readline()
                    except:
                        print("")
        except:
            print("")
...
def current_receive():
    global ser_st,ser_screen,current,max_current,power,max_power
    while True:
        try:
            if ser_st.in_waiting > 0:
                receive_data1 = ser_st.readline().decode('utf-8').strip()
                # print(f"收到串口数据：{receive_data1}")
                header, data = parse_received_data_float(receive_data1)
                if header == "C":
                    current = float(data)
                    power = current * 5.0
                    if max_power<power:
                        ...
                        if max_power>7.135:
                            max_power=7.135
                    try:
                        cmd=f"Current={int(current*1000)}"
                        cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                        ser_screen.write(cmd_bytes)  # 发送数据到串口
                        # print(cmd_bytes)
                        time.sleep(0.001)
                        cmd = f"Power={int(power * 1000)}"
                        cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                        ser_screen.write(cmd_bytes)  # 发送数据到串口
                        time.sleep(0.001)
                        cmd = f"PowerMax={int(max_power * 1000)}"
                        cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                        ser_screen.write(cmd_bytes)  # 发送数据到串口
...
if __name__ == "__main__":
    ...
    current_thread = threading.Thread(target=current_receive, daemon=True)
...
#
#
#         res = find_and_draw_outer_black_rectangle(thresh,frame,False,False)
#
#         if res is not None:
#             #print("检测到矩形外框")
#
...
#             if triangle_length is not None:
#                 triangle_lengths.append(triangle_length)
#             rectangle_length = find_rectangle_points(thresh_res, res[0], True, True)
#             if rectangle_length is not None:
#                 rectangle_lengths.append(rectangle_length)
#             count += 1
#             sums+=res[3]
