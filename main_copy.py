...
def process_command(cmd):
    ...
    if cmd.startswith("add") and "=" in cmd:
        ...
        if idx_str.isdigit() and val_str.lstrip("-").isdigit():
            idx = int(idx_str)
            delta = int(val_str)
            modify_param_by_index("/home/Hust_HWD/Desktop/ti_cup/params.txt", idx, delta)
        else:
...
def main_task():
    global ser_screen,ser_st,ser_tiaoshi,cap,i,radiuses,triangle_lengths,rectangle_lengths,diagonals,return_list_all,number_list,list_lunci,question,number,current,max_current,power,max_power
    while True:
        while True:
            try:
        ...
        if question == 1:
            ...
            try:
                if len(diagonals) >0:
                    ...
                    # 使用 struct 打包为 4 字节小端 float
                    data_bytes = struct.pack('<f', round(X *100.0))  # IEEE 754 格式，小端字节序
                    print("X:", round(X / 10.0, 3))
                    cmd = f"Length={int(X *100)}"  # 构造命令字符串
                    cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                    ser_screen.write(cmd_bytes)  # 发送数据到串口
            ...
            if ser_st.in_waiting > 0:
                ...
                if header == "C":
                    ...
                    try:
                        ...
                        ser_screen.write(cmd_bytes)  # 发送数据到串口
                        time.sleep(0.001)
                        cmd = f"MaxPower={int(max_power * 1000)}"
                        cmd_bytes = cmd.encode('utf-8') + b'\xff\xff\xff'  # 编码成字节串并添加结束符
                        ser_screen.write(cmd_bytes)  # 发送数据到串口
