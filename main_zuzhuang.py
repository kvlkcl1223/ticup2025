...
# 专门对重叠图形进行判断
def is_square(points, angle_thresh=15, side_ratio_thresh=0.15):
    """
    判断4个点是否近似构成正方形
...
while True:
    ...
    if ret:
        ...
        cv2.imshow("framewaesrfyui", thresh)
        cv2.waitKey(1)
        # 开闭运算去噪
        # kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        ...
        if res is not None:
            ...
            # count+=1
            # if count%10==0:
            #     print(f"平均对角线长度: {np.mean(diagonals):.2f}")
            #     diagonals=[]
            ...
            # print("圆半径", find_circles(gray_res,thresh_res,res[0],True,True))
            # print("三角形边长", find_triangle_points(thresh_res, res[0], True, True))
            # print("正方形边长", find_rectangle_points(thresh_res, res[0], True, True))
